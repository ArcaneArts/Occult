...
Command-line interface for occultist built with the Arcane Templates CLI framework.

## Overview

This is a Dart-based CLI application that uses `cli_gen` for beautiful, type-safe command generation. The CLI integrates with your occultist ecosystem including models, server API, and Firebase services.
...
To use `occultist` command globally on your machine during development:
...
occultist --help
occultist hello greet "World"

# Deactivate when done
dart pub global deactivate occultist
```
...
# Add your repo info:
homepage: https://github.com/YOUR_USERNAME/occultist
repository: https://github.com/YOUR_USERNAME/occultist
issue_tracker: https://github.com/YOUR_USERNAME/occultist/issues
...
dart pub global activate occultist
...
Users update with:
```bash
dart pub global activate occultist  # Gets latest version
```
...
```bash
# Greet someone
occultist hello greet "Alice"

# Multiple greetings with enthusiasm
occultist hello greet "Bob" --times 3 --enthusiastic

# Show version
occultist hello version
```

### Config Commands

Manage application configuration (stored in `~/.occultist/config.yaml`):

```bash
# Initialize config file
occultist config init

# Set a value
occultist config set server_url https://api.example.com
occultist config set api_key your_secret_key

# Get a value
occultist config get server_url

# List all config
occultist config list

# Show config file path
occultist config path
```
...
import 'package:occultist_models/occultist_models.dart';
...
```bash
dart run build_runner build --delete-conflicting-outputs
occultist data list-users --limit 5
occultist data create-user "Alice" "alice@example.com"
```

### Server Commands (If Enabled)

Call your occultist server API:

```bash
# Ping server health check
occultist server ping

# Get server info
occultist server info

# Configure server connection
occultist server configure --url https://your-server.com --key your_api_key

# Test authenticated API call
occultist server test
```

**Configuration:**
- Server URL: Set via `--url` flag, environment variable `occultist_SERVER_URL`, or config file
- API Key: Set via `--key` flag, environment variable `occultist_API_KEY`, or config file
...
├── lib/
│   ├── occultist.dart           # Main CLI runner
...
2. **Register in Runner** (`lib/occultist.dart`):

```dart
import 'commands/my_command.dart';

@cliRunner
class occultistRunner extends _$occultistRunner {
  // ...
...
export occultist_SERVER_URL=https://api.example.com
export occultist_API_KEY=your_secret_key
...
Config is stored in `~/.occultist/config.yaml`:

```yaml
# occultist Configuration
app_name: occultist
version: 1.0.0
...
- Check that server is running: `occultist server ping`
...
Generated from Arcane Templates - Part of the occultist project.
