...
class WizardConfig {
  ...
  Map<String, String> toDisplayMap() {
    return {
      ...
      if (useFirebase && firebaseProjectId != null && firebaseProjectId!.isNotEmpty)
...
class WizardValidators {
  ...
  static ValidationResult validateAppName(String name) {
    ...
    if (RegExp(r'^[0-9]').hasMatch(name)) {
      return const ValidationResult.invalid('App name cannot start with a number');
    }
  ...
  static ValidationResult validateFirebaseProjectId(String id) {
    if (id.isEmpty) {
      return const ValidationResult.invalid('Firebase project ID cannot be empty');
    }

    if (id.contains(' ')) {
      return const ValidationResult.invalid('Firebase project ID cannot contain spaces');
    }

    if (id.length < 6) {
      return const ValidationResult.invalid('Firebase project ID must be at least 6 characters');
    }

    if (id.length > 30) {
      return const ValidationResult.invalid('Firebase project ID must be at most 30 characters');
    }
  ...
  static ValidationResult validateOrgDomain(String domain) {
    if (domain.isEmpty) {
      return const ValidationResult.invalid('Organization domain cannot be empty');
    }

    if (domain.contains(' ')) {
      return const ValidationResult.invalid('Organization domain cannot contain spaces');
    }

    final parts = domain.split('.');
    if (parts.length < 2) {
      return const ValidationResult.invalid('Organization domain needs at least 2 parts (e.g., com.example)');
    }
